...
<p>enter new password</p><br/><br/>
<input type='text' name='password' placeholder='your new Password'/><br/><br/>
...
$otp1 = mysqli_real_escape_string($conn, $_POST['otp']);
$otp= filter_var($otp1, FILTER_SANITIZE_STRING);
$password1 = mysqli_real_escape_string($conn, $_POST['password']);
$password= filter_var($password1, FILTER_SANITIZE_STRING);

if(strlen($password) < 8){
echo '<h3> your password must cuntain atleast 8 letters </h3>';

}
elseif(preg_match("#[0-9]+#",$password) == false ){
echo '<h3> your password must cuntain atleast one number </h3>';

}
elseif(preg_match("#[A-Z]+#",$password) == false ){
echo '<h3> your password must cuntain atleast one Capital Letter </h3>';

}
elseif(preg_match("#[a-z]+#",$password) == false){
echo '<h3> your password must cuntain atleast one small Letter </h3>';

}
elseif($otp <> 0){

$sql1 = "SELECT * FROM vidyarthi WHERE email= '$email'";
...
$pub= "UPDATE vidyarthi SET password = '$password' WHERE email = '$email' ";
mysqli_query($conn, $pub);
$pub1= "UPDATE vidyarthi SET otp = '0' WHERE email = '$email' ";
